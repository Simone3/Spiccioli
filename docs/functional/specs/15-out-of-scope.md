...
| Showing an oversold position | A (security, account) whose running quantity is negative gets no holding row and no derived figures — average cost, invested and gain are all undefined on it. Checks 8 and 9 name the trade instead, which is the report the situation actually calls for ([§2](02-domain-model.md), [§9](09-checks.md)). |
