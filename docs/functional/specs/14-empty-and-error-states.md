...
behind it, and every one of them says what to do next rather than showing a blank card.
...
- **Empty because of a filter is a different state** from empty because nothing exists. The first
  says which filter is hiding the rows and offers to clear it; the second is the state above.
...
| Investments › Holdings | No trades: “Record a purchase to start tracking holdings,” with the button. Trades but no prices: holdings render at `€ 0,00` with the price cell reading *none* and inviting a value, and check 3 says why ([§11.3](11-calculations.md#113-hypothetical-liquidation)). | A fetch always ends in the same review panel, whether it went well or badly: what would be written and over what, which securities had no quote, which could not be fetched at all, naming them with their reasons, and the provider's reference date ([§7.6](07-investments.md#76-prices)). Confirm and a line says what was written; cancel and nothing was. A provider that is unreachable altogether is that panel with nothing to write and only *Close* on it. The rest of the screen works throughout and every known price stays. |
...
- **Empty-because-filtered and empty-because-nothing-exists are kept apart** because confusing the
  two makes a user hunt for data that was never there.
- **The two blocking errors are the exceptions because neither leaves anything worth doing.** Every
