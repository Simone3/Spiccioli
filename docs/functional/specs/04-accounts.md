...
- A `Brokerage` account shows an em dash for opening balance and for transactions, and every cash account shows one for trades: neither can hold what the other does ([§2](02-domain-model.md)). The form enforces it ([§13](13-validation.md)).
...
- The list carries the name, the **default sell fee** and an **Accounts** count: the accounts pointing at it, open and closed alike. That count is what says whether an institution can be deleted — at zero it can, otherwise it cannot.
