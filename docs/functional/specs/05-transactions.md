...
- **Matched** is derived and read-only, and it names the counterpart for **every** kind of pairing [§11.6](11-calculations.md#116-derived-matching) makes: the counterpart account for a paired internal transfer, the security for a trade-matched securities transaction, the **payslip** for a salary transaction paired with one — its month and its label, *December 2025* or *December 2025 · 13th* — and, for a pension-fund credit, the **contribution period** whose figure claimed it: *December 2025* where a credit covers one month, *October 2025 – December 2025* where it covers a quarter ([§11.6](11-calculations.md#116-derived-matching)). An em dash otherwise.
...
- **50 rows per page.** This is one of the two paginated tables in the application — the other is a security's price history, which pages at twenty in a panel of its own ([§7.4](07-investments.md#74-securities)) — and every other table is shown in full.
- **The screen opens on the first page**, so the most recent rows are in view. The pager is how you go back. That is the first page of what the filters match on a screen opened for the first time and on one arrived at by a link; a screen come back to from the sidebar reopens on the page that was in view when it was left ([§12.2](12-storage.md#122-the-menu-bar-and-which-file-is-open)).
- **The pager is five controls**: first, previous, the page in view as a box that can be typed into, next, last. A page number typed outside the range is refused and the box goes back to the page in view; nothing is clamped. **It states how many pages there are** and nothing else — how many rows a page holds is fixed above and is not a thing the pager repeats. **It is the same pager wherever a table pages**, the price history included: the page length is the table's and the five controls are not.
- **Changing a filter lands on the first page of what it now matches**, on the same rule and for the same reason. A page number carried over from the previous filter would point at a different part of a different list — page 7 of nine hundred rows is somewhere in 2019, and page 7 of eighty is the end — so the position is recomputed rather than kept. **This is the only thing a filter change does to the view**, the ordering and everything else being fixed ([§5.6](#56-selecting-and-deleting-in-bulk) is what it does to the selection).
- **However the screen is reached, it is the same screen.** Arriving from a report cell ([§6.1](06-categories.md#61-report)), from a finished import ([§5.7](#57-bulk-import)) or from a failing check's entry ([§9](09-checks.md)) sets the filters and changes nothing else: the first page of what those filters match, the same ordering, the same everything. **A screen reached that way is a fresh arrival** and keeps nothing of what it was showing before ([§12.2](12-storage.md#122-the-menu-bar-and-which-file-is-open)) — the handed filters are the whole of what is set, never an addition to filters left over from earlier.
...
- **The list is narrowed by typing**, one bank being able to have several templates and a list of them all being one nobody reads to the end of. **Typing narrows and never chooses**: a template is chosen by pressing it, so a search that happens to leave a single entry still takes the press that says it was the one meant — and a template the search has narrowed out of sight stops being chosen, so what was pressed and what is read can never be two different things.
- **A template says what it reads, and the application reads exactly that.** Which sheet, how the rows of figures are found under whatever the export prints above them, which columns carry the date, the description and the amount, and — because an export is not obliged to write them the way this file does — **what its dates and its separators mean**. Nothing is detected from the data: as with the three controls, the user names the shape and the application obeys.
- **A description may be several columns joined**, a bank routinely printing one across two or four of them, and a column that is empty on a row simply is not there — the join never leaves a separator dangling on nothing.
...
- **Some exports print their headings again before every row**, with rows of detail under each that this import has no column for. Where a template says so, the rows read are the ones directly under a heading row and **nothing else on the sheet is a row at all**.
- **Where an export writes more than a date in its date column** — a timestamp, most often — the template says what part of it is the date. What is left over is dropped before the box ever sees it, the box reading a date and never a time beside one ([§15](15-out-of-scope.md)).
...
- **An amount may be one signed column, a debit-and-credit pair, or a count and a unit price.** Where a template names a pair, the two are read into the one signed figure the box carries — money out negative, money in positive — and a row carrying a figure in both columns cannot be read. Where it names a count and a price, which is how a voucher account states a movement, the two are multiplied — exactly, a whole number of cents by a whole count — and **the direction comes from a column the template names**, against a closed list of what that column may say. **A row whose column says something the list does not carry keeps the cell as the export wrote it** and is marked in the preview: a movement the bank has just invented is a row to look at, never a figure pointed the way that was convenient.
...
- **The chooser opens in the downloads folder the first time it is opened**, an export that has just been downloaded being there, and **where the last one was taken from every time after that**. It is remembered for as long as the application is running and no longer: a file a statement was imported from last month is not where the next one will be, and a folder remembered across runs would be a stored setting, which [§10](10-settings.md) has no place for. **A file the template then refuses still moves it**, the folder being where the user went whether or not what they found there could be read.
...
- **Zero, one or two decimals are all read**: `1234`, `1234,5` and `1234,56` are amounts. **Places past the cent are read too, as long as they are zeros**: a bank writing `3,860000` is writing € 3,86 in a longer hand, and the row is read as € 3,86 rather than marked. **A non-zero place past the cent cannot be read** — `1234,567` is an unreadable row, exactly as the amount field refuses a third decimal ([§13](13-validation.md)). Nothing is ever rounded: a figure the file has no room for is refused, never quietly shortened.
...
- **A `€` or `EUR` marker on the amount is stripped before the separators are read**, leading or trailing, with or without a space between it and the digits: `€ -54,80`, `-54,80 €` and `-54,80 EUR` are the same amount as `-54,80`. **Those two spellings and no others.** **The sign may sit on either side of a leading marker**, with or without a space after it: `-€ 54,80`, `- €54,80` and `€ -54,80` are all the same amount too. It is still one sign and still a leading one — a field carrying a sign on both sides of the marker cannot be read. This is not configurable and there is no fourth control: there is no currency setting anywhere in the application ([§10](10-settings.md)), and `€` is simply what the amounts in this file are ([§1](01-premise-and-constraints.md)).
