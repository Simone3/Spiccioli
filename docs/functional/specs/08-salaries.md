...
- A payslip is corrected on the form it was recorded on, opened by **Edit** in its row menu, which also holds **Duplicate** and **Delete**. The fields are the same either way and so is the record. Duplicate copies every field including `year`, `month` and `label` — a month may legitimately hold more than one payslip ([§2](02-domain-model.md)), so the copy is valid where it lands. **Add payslip** opens that same form with **all twelve stored fields**, `year` at the top and the eleven the table shows below it. **Year is the one field that arrives filled in**, and it is a picker holding exactly the years the per-year table has rows for — the contract's own years, so no year the contract never covered can be chosen. **It opens on the year whose row is selected in that table**, which is the year the payslip table below is already showing; **a row is always selected** — on arriving at the screen it is the contract's last year — so the field always opens on something. Changing it is how a payslip is filed into another year without leaving the form, and **saving into a year other than the selected one moves the per-year selection to that year**, so the new row is visible where it landed rather than in a table it is not in. **Every other field is empty**: no month is proposed, no figure is carried over from the previous payslip, and **no amount on the form is filled in on the user's behalf** ([§5.5](05-transactions.md#55-add-transaction)). Duplicate is what exists for a row that resembles another.

> **Mockup —** [Add payslip](../mockups/08-salaries.html#add-payslip)
