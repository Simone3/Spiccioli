...
- **Once the import's three controls have said what a row's characters mean, the paste and the form admit exactly the same values.** A typed `1234` and a pasted `1234` are both `€ 1.234,00`; a non-zero third decimal is refused on both paths. The asymmetries run the harmless way and there are two of them: the amount field will not accept a thousands separator at all, while a paste may carry them, since a field knows there is none in play and a bank export does not say so until the control does; and a paste may carry zeros past the cent, `3,860000` reading as `€ 3,86`, since an export writes the places its own format has while a field is typed one character at a time. Neither admits a value the other would have turned into a different figure ([§5.7](05-transactions.md#57-bulk-import)).
...
| pensionContributionMonths | One of **1, 2, 3, 4, 6, 12** — the divisors of 12, which are the period lengths that can be anchored to the start of a year without straddling its end ([§11.6](11-calculations.md#116-derived-matching)). A closed set, offered as a picker and never typed. |
