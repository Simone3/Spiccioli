...
| type | Required. **The five cash types are freely interchangeable at any time**; the boundary between them and `Brokerage` is **locked from the moment the account exists**, empty or not. An account created as `Brokerage` stays a `Brokerage` one and an account created as cash stays cash — the picker offers only the side the account is already on. To move one across that boundary, delete it and record it again. |
