...
- **`stubLedgerBridge` stands in for the preload.** Every channel it publishes is answered, and a test overrides only the ones it cares about — so a channel added to the bridge is a compile error here rather than an undefined function at run time.
