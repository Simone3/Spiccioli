...
- **A chart needs `stubChartLayout` before it draws anything.** The charting library measures the box it is put in, and jsdom lays nothing out and has no `ResizeObserver`; the factory stands both in, and every test that asserts on a drawn chart calls it from `beforeAll`.
...
- **A path is made by the host and never written out.** `.github/workflows/release.yml` runs these on Windows as well as on macOS and Linux, and there `path.resolve` answers `/Documents/finances.spiccioli` with the current drive in front of it. So a test that expects a path builds it through `node:path` rather than spelling it: what is under test is which path was picked and how the platform compares two of them, and the shape of an absolute one is the host's own business. **A path standing in for one the application already holds is resolved too** — a session holds what a dialog gave it, so comparing that against a resolved path is only honest if both sides are. The same care covers a document read off disk, a Windows checkout writing it with CRLF endings.
...
- **`SamplePayslips.test.ts` is the same thing for a payslip**, and is the one test that runs under Node rather than under a browser — a PDF is never opened in the window, so the reader it exercises is the main process's. `scripts/write-sample-payslips.js` writes the documents, `PdfText` takes the lines of text and the points they sit at out of the bytes, and the shipped template finds each figure in the box its heading opens. Three documents are written and the second and third are why: one prints neither the expenses nor the car, so the half of the template that reports a figure the document never carried is exercised too, and one is a *tredicesima*, whose period box names the payment instead of the month and which has to land in December carrying a label. A `// @vitest-environment node` comment at the top of the file is what puts it there.
- **`SampleImports.test.ts` is the one place the import's four steps meet.** `scripts/write-sample-imports.js` writes one sample bank export per template, the workbook or delimited reader takes a grid out of the bytes, that template turns the grid into text for the paste box, and the paste parser reads that text. A template exists to produce rows the box accepts, so a template whose rows the box marks is a template that does not work — and nothing below this test would have said so. **Each fixture is that bank's real shape with invented figures**, which is what lets the awkward parts — preamble rows, repeated headings, a date written as text, a timestamp where a date belongs — be tested without a real export, which is somebody's spending, going into the repository.
