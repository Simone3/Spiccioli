...
| `main/config/BackupLocationManager.ts`, `main/storage/BackupDirectory.ts`, `main/ipc/BackupLocationIpc.ts`, `types/BackupTypes.ts` | **Do not apply.** They exist to let the user choose a backup folder, to refuse an unusable one, and to hold how many copies that folder keeps. Spiccioli never asks for any of it: the backup folder is derived from the ledger's own name and directory ([§12](../functional/specs/12-storage.md)) |
| `main/storage/DatabaseStorage.ts`, `main/storage/DatabaseBackup.ts`, `main/storage/BackupScheduler.ts`, `main/storage/InvalidChangeError.ts`, `main/ipc/StorageCommandIpc.ts`, `renderer/StorageQueue.ts`, `types/StorageTypes.ts` | **Do not apply as they are.** They implement one write per user command against a database that never moves, with one copy in the chosen folder kept up to date and dated copies taken from it on a cadence. Spiccioli autosaves a whole file, debounced and atomically, retries a failed write five times, detects that something else changed the file, and takes one backup per session on close. **The second storage layer sits beside these rather than replacing them** (D5, D6): the two models coexist in the folder, each with its own tests, and the whole-file one is what [§12](../functional/specs/12-storage.md) is built on |
