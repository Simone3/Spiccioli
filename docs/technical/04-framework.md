...
**None of it is deleted, and that is now decided rather than deferred** (D6, [§8.2](08-implementation-plan.md#82-decisions)). [§4.1](#41-what-it-is) keeps this folder byte-identical to SPOT's copy on purpose, and removing the modules Spiccioli happens not to use would diverge the two for no gain — the next fix carried between them costs more than the idle code does. A module that does not apply says so here and stays compiled, linted and tested.

| Module | Status |
| --- | --- |
| `utils/DateUtils.ts` | **In use.** It parses and writes the stored `YYYY-MM-DD` day every screen reads, and Phase 10 added the day arithmetic the match windows and the check ages are built on — `addDays`, `addMonths` with its clamp, and `dayDifference`. **Printing a day is not its job**: that goes through `src/logic/format/DateFormat.ts`, because the format is a preference rather than a locale |
