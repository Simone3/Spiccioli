...
| `ipc/ImportIpc.ts` | The one channel a file an import reads crosses — a bank export ([§5.7](../functional/specs/05-transactions.md#57-bulk-import)) or a payslip ([§8.1](../functional/specs/08-salaries.md#81-payslips)): the chooser, the bytes, and a grid of cell text back. **The path never crosses**, and nothing but a path, a count and a reason reaches the log. It holds one thing between calls — the folder the last file came from, **one per import and not one between them**, which is where the next chooser opens and which lives no longer than the run |
