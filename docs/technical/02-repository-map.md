...
| `ipc/ImportIpc.ts` | The channels a file an import reads cross — a bank export ([§5.7](../functional/specs/05-transactions.md#57-bulk-import)) or a payslip ([§8.1](../functional/specs/08-salaries.md#81-payslips)): the chooser, the bytes, and a grid of cell text back. **Two of them, one document and several**, sharing the one read: the second opens a chooser that takes a selection and answers with **one outcome per document**, a document nothing could be opened out of being one marked row of the recap rather than the end of the selection. A selection larger than an import reads at a time is the one refusal the whole of it takes, and nothing is read for it. **The path never crosses**, and nothing but a path, a count and a reason reaches the log. It holds one thing between calls — the folder the last file came from, **one per import and not one between them**, which is where the next chooser opens and which lives no longer than the run |
| `import/ZipArchive.ts` | As much of the zip container as opening a workbook takes: the central directory, and one member's bytes by name. Written rather than installed, which is what keeps [§8.3](08-decisions.md#83-the-dependencies-the-application-adds) as short as it is |
| `import/XlsxGrid.ts` | A sheet read out as the text its cells hold — the sheet list, the shared strings, inline strings, and a cell's value. **A number comes back as the literal the file spells it with**, never as a `number`, so the conversion to cents stays where [§8.2](08-decisions.md#82-what-d3-fixes) puts it |
| `import/DelimitedGrid.ts` | A delimited export split on the character its template names, quoting and all, decoded under the encoding the template states — UTF-8 or the Windows code page, never sniffed |
| `import/PdfText.ts` | A document read out as the lines of text it prints, which is what a payslip is read from. **The one module that imports `pdfjs-dist`** ([§8.3](08-decisions.md#83-the-dependencies-the-application-adds)): the pieces of text on a page are grouped back into lines by the baseline they sit on, **each keeping the point it starts at across the page** — which is the whole of what tells a withholding from a credit on a payroll form. Nothing is ever rendered, and the library is told it may not reach the network |
...
| `components/import/` | Bulk import ([§5.7](../functional/specs/05-transactions.md#57-bulk-import)): the paste, the three format controls, the account, the outcome, and the preview table that marks every row — plus *Upload* and the template picker that fills the box from a bank export and moves the three controls to what that template declares. **`TemplateChooserDialog` is that picker and is both imports'** — a list of names narrowed by typing, knowing nothing about what a template reads, and taking the wording of the button that opens the file chooser as a parameter, one import choosing a file and the other a selection of them |
...
| `components/salaries/` | The Salaries screen ([§8](../functional/specs/08-salaries.md)): the two tabs, the contract selector that scopes the first of them, the contracts table and its form, the per-year table whose working-days cell opens the one-field form that **is** the ContractYear record, the twelve-column payslip table and its form, and the two charts. *Import payslip* reads a document onto that same form and writes nothing itself: what a template found arrives in the fields, what it did not is empty and named in the notice above them, and the payslip exists once the form is saved. **Several documents open `PayslipImportRecap` instead**, which is a list and not a form — one row per document stating the payslip it would write, a marked row for each one that cannot be written, a tick on the rest, and one save for all of them |
...
| `logic/salaries/PayslipImportBatch.ts` | A selection of payslip documents turned into the rows the recap ticks and writes ([§8.1](../functional/specs/08-salaries.md#81-payslips)): one row per document, ordered the way the payslip table orders its own, with the refusals stated per document and never for the selection. **The one place in the application a figure nobody typed is written** — what a template did not find becomes a zero, except in the two figures [§13](../functional/specs/13-validation.md) refuses a zero in, where the row cannot be ticked at all. A payslip the file already holds, or one an earlier row of the same selection would write, is flagged and arrives unticked: a flag and not a refusal, a month legitimately holding several |
...
| `logic/import/PayslipTemplate.ts` | What a payslip template is and what applying one to the lines of a document produces: the line the period is on, **the heading each figure's box is under**, and where a document says the payslip is one of a kind that carries a name — **which name that is comes in as a parameter**, a payroll's own shorthand for a *tredicesima* being no use to anybody reading the Payslips table and wording belonging in the bundle ([§5](05-text-and-languages.md)). **A payslip is a form of boxes, so a figure is found by its heading and not by its line**: the column a heading opens is measured off the heading line and the figure is the one printed inside it, which is the only thing that tells two identically printed figures apart. **What it produces is the values the payslip form opens on** — never a payslip, which exists once that form is saved. A figure it did not find is an empty field and is named, never a zero |
| `logic/import/PayslipTemplates.ts` | The payslip templates the application ships, which is Reply's Italian payslip — the `Mod. Cedolino TS` form TeamSystem's payroll prints. Three figures come out of boxes of their own and five out of the `DESCRIZIONE VOCE` table, each from the column that says what it is; the `MESE RETRIBUITO` box carries both the period and, on a *tredicesima*, the label — that being the one payslip the form names after the payment rather than after the month. Each side of the pension fund is printed under either of two labels and a month printing both has them summed, exactly as the two car lines are. Code and not configuration, the same as the bank templates above |
...
| `write-sample-payslips.js` | Writes the sample payslips the shipped payslip template reads — an invented employer's documents on the real form, with every piece placed at the point it would really be printed at, written by a PDF writer of its own and importing nothing from `src`. `tests/main/SamplePayslips.test.ts` runs it and takes each one through the application's own reader and template |
