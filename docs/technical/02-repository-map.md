...
| `import/PdfText.ts` | A document read out as the lines of text it prints, which is what a payslip is read from. **The one module that imports `pdfjs-dist`** ([§8.3](08-decisions.md#83-the-dependencies-the-application-adds)): the pieces of text on a page are grouped back into lines by the baseline they sit on, nothing is ever rendered, and the library is told it may not reach the network |
...
| `logic/import/PayslipTemplate.ts` | What a payslip template is and what applying one to the lines of a document produces: the line the period is on, the line each figure is on, and the name a document prints for a month that has one. **What it produces is the values the payslip form opens on** — never a payslip, which exists once that form is saved. A figure it did not find is an empty field and is named, never a zero |
| `logic/import/PayslipTemplates.ts` | The payslip templates the application ships, which is the sample one `scripts/write-sample-payslips.js` writes documents for. Code and not configuration, the same as the bank templates above |
...
| `write-sample-payslips.js` | Writes the sample payslips the shipped payslip template reads — an invented employer's documents, written by a PDF writer of its own and importing nothing from `src`. `tests/main/SamplePayslips.test.ts` runs it and takes each one through the application's own reader and template |
