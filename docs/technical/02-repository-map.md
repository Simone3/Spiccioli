...
| `prices/PricePass.ts` | One pass, end to end: one paced request per listing over the window it carries, each failing on its own, the answers it reports as it goes, the flag that abandons it, and the three log entries D14 fixes |
...
| `components/transactions/` | The Transactions screen ([§5](../functional/specs/05-transactions.md)): the list, the seven filters, the pager, and the one form a row is both recorded on and corrected on |
...
| `components/checks/ChecksScreen.tsx` `.css` | The Checks screen ([§9](../functional/specs/09-checks.md)): the fourteen in the order of the specification's table, each stating its reach, each failing one naming at most five records with the whole count and a link per record |
