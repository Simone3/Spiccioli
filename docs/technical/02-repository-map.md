...
| `ipc/ImportIpc.ts` | The one channel a bank export crosses ([§5.7](../functional/specs/05-transactions.md#57-bulk-import)): the chooser, the bytes, and a grid of cell text back. **The path never crosses**, and nothing but a path, a count and a reason reaches the log. It holds one thing between calls — the folder the last export came from, which is where the next chooser opens and which lives no longer than the run |
| `import/ZipArchive.ts` | As much of the zip container as opening a workbook takes: the central directory, and one member's bytes by name. Written rather than installed, which is what keeps [§8.3](08-decisions.md#83-the-four-dependencies-v1-added) closed at four |
...
| `components/import/` | Bulk import ([§5.7](../functional/specs/05-transactions.md#57-bulk-import)): the paste, the three format controls, the account, the outcome, and the preview table that marks every row — plus *Upload* and the template picker — a list of the shipped templates narrowed by typing — that fills the box from a bank export and moves the three controls to what that template declares |
