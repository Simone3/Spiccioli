...
- **`node_modules` is excluded except for `electron-log`.** Everything else is already inside the two bundles: Vite bundles the renderer's dependencies into `build/` — fonts and all, as emitted assets — and esbuild bundles the main process's into `dist/electron/main.js`, the one file bundling cannot reach being the PDF library's worker, which is copied beside it ([§3.3](#33-two-bundles-two-bundlers)). **`electron-log` is the exception because it is external to the esbuild bundle** and is therefore the only thing a packaged run resolves out of `node_modules` at all; `electron` resolves to the runtime's own module and never to a folder here. The archive is **8.7 MB rather than 113 MB**, and what was being carried was a second copy of code already inside the bundles.
- **A dependency the bundlers cannot reach has to be named beside `electron-log`**, in `forge.config.js`, or a packaged run fails on a module a development run resolves. **What says whether one is reachable is the bundle itself**: a bare `require` in `dist/electron/main.js` or `dist/electron/preload.js` that is neither a Node built-in nor `electron` is a module the package has to carry.
