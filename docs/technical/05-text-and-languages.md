...
- Numbers interpolated into a translation are formatted with `Intl.NumberFormat` for the same locale, so a translated sentence and the numbers inside it can never disagree. **That locale is not the preferences**, so a figure the user reads is formatted by `Formatter` first and interpolated as a string — a position in a refused file is written `2.857th` and never `2,857th`.
