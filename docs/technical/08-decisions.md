...
## 8.3 The dependencies the application adds

**Four libraries were the whole of what v1 added** to the five runtime dependencies `package.json` started with, and **a fifth arrived after it**, with the payslip import of [§8.1](../functional/specs/08-salaries.md#81-payslips). **Every one is pinned to an exact version**, and the tree each drags in is a large part of why the set is as small as it is.
...
| `recharts` | 3.10.1 | `@reduxjs/toolkit`, `react-redux`, `immer`, the d3 modules of `victory-vendor` and a handful of small utilities — much the largest tree of the four v1 added |
| `pdfjs-dist` | 6.3.289 | Nothing at all: it is one project, published as a build with no dependencies of its own — and much the largest package of the five, most of which is a viewer, a set of character maps and a set of standard fonts that none of this uses |

**Every one of them is wrapped where it is used**, which is what keeps the choice reversible: the date picker behind our own control (D9), each chart behind a component of its own, the drag behind the rule list, and the PDF reader behind `src/main/import/PdfText.ts`, which is the one module that imports it and which hands back an array of strings. **What the rest of the code imports is Spiccioli's own module and never the library**, so replacing any of the five is a change in one folder.
...
**The set stayed at four when the transactions import learned to read a file** ([§5.7](../functional/specs/05-transactions.md#57-bulk-import)), and the reason is not only the count. An `.xlsx` is a zip of XML documents, and the part of it a table of rows uses — the sheet list, the shared strings, a cell's value — is a few hundred lines, so `src/main/import/` reads one itself. **What a library would have cost is the boundary D3 depends on**: every spreadsheet reader hands back a JavaScript `number` for an amount cell, which is a binary double that has already been rounded, and converting *that* to cents would put a rounding between the file and the import parser that [§8.2](#82-what-d3-fixes) names as the one place the conversion happens. Reading the literal text out of the document keeps the parser the boundary and lets a figure the file has no room for be refused rather than quietly shortened — which is what [§5.7](../functional/specs/05-transactions.md#57-bulk-import) already promises about a paste. The same goes for the delimited half: the quoting rules are twenty lines, and the encoding is the template's to state rather than something to sniff.

**The fifth was taken for a shape nothing sensible writes by hand.** A payslip is a PDF, and reading the text out of one is not a few hundred lines the way a sheet of a workbook is: the format carries its objects in compressed streams behind a cross-reference table, its text in operators that place pieces of a line at coordinates, and its characters behind whatever encoding each embedded font declares — and a payroll provider's document uses all of it. **What the boundary argument above says about a spreadsheet does not apply here**: a text extractor hands back the characters the page prints, which is exactly what `src/main/import/` already hands the templates, so the conversion to cents stays in the one parser [§8.2](#82-what-d3-fixes) puts it in. Two things are held to instead. **Nothing renders**: only the text layer is asked for, no font is installed or drawn with, and the library's own console is turned down to its errors. **Nothing leaves the machine**: the library is able to fetch character maps and font data over the network and is told it may not, which keeps D11 exactly where it was.

**The build carries one file the bundler does not.** The library reads a document in a worker it loads by importing a file beside whatever pulled it in, and esbuild bundles the library without knowing about that import — so `scripts/electron-bundle.js` copies `pdf.worker.mjs` next to the main bundle, in the one-shot build and in the development loop alike. Without it the first payslip anybody imported would fail on a file that was never there. **The package therefore carries nothing out of `node_modules` for it**, which is the rule every dependency but `electron-log` is now under ([§3.6](03-build-and-run.md#36-packaging)): the installed copy is 33 MB of viewer, character maps and standard fonts that nothing here loads.

**It is imported when a document is read and not when the process starts.** A megabyte of code that announces on the console what it would need to draw a page with is not something a session that opens a file and looks at it should be carrying, so `readPdfLines` imports it inside itself. This is the one dynamic import in the application, and what it buys is a start-up that is exactly what it was before the payslip import existed.
...
**The log is one local NDJSON file and nothing sends it anywhere** ([§4.4](04-framework.md#44-the-logger)) — there is no telemetry, no crash reporting service and no console an installed Spiccioli can open, so this file is the whole account of a run that survives it. **Settings names the folder it is in**, read-only, and that is the whole of the interface it has: nothing opens it, empties it or offers to send it anywhere. Every entry is therefore written to be legible on its own, by whoever collects the file after something went wrong.

**Four levels, meaning the same thing everywhere.** `debug` for what happens on a timer or a keystroke — a save, one listing fetched, one HTTP exchange with the price provider. `info` for the moments of a session — a file opened, created, upgraded, closed, a copy written. `warn` for something refused or recovered — a retried write, an external modification, a file the reader would not take. `error` for something lost — five failed attempts, a copy not written, a render error, a crash.

**Which of them are written is the `logLevel` preference of [§10](../functional/specs/10-settings.md), and it defaults to `info`.** A level admits itself and everything more severe, so the default writes the three moments-and-failures levels and drops the tracing; **an entry below the level in force is never serialized at all**, which is what makes `debug` affordable to leave in the code. The filter is the framework logger's own and applies to every entry uniformly, `config.startup` included — a run kept at `warn` therefore has no line describing itself, which is why that entry carries the level it was written under. Changing the preference applies to the next entry rather than to the next run: the renderer writes it, and `Main.ts` hands the new level to `appLogger.setLevel`.
...
**The price pass.** The two entries about a listing are written where the adapter is called; **the exchange itself is written inside it**, because the request, the response and the milliseconds between them exist nowhere else. The split is deliberate: `PricePass.ts` says what a listing came to once [§7.6](../functional/specs/07-investments.md#76-prices)'s refusals have been applied to it, and `YahooPriceProvider.ts` says what actually went over the wire.
...
| One listing | `prices.listing` | `debug` | The `ticker`, the `exchange`, and what came back **once it has been read**: how many days it carried, how many were dropped and the newest day of them, or which of the four refusals of [§7.6](../functional/specs/07-investments.md#76-prices) it fell to, or the transport failure. This is the outcome, and the entry below it is the wire |
| One HTTP exchange | `prices.http` | `debug` | **The whole request and the whole response**, written from inside the adapter because that is the only place that has them: the method, the URL, the two request headers, the timeout it was given, the status, whether it was OK, the response headers, the body and **how many milliseconds the round trip took**. A request that never completed is written the same way, with `null` for the status and what went wrong in its place. **The body is cut at `PRICES_CONFIG.maximumLoggedBodyLength`**, with its real length and whether it was cut beside it — a span of years is several thousand days, and a pass over a whole file would otherwise spend a good part of the size limit in one press. **Anything cookie-shaped is redacted**; everything else in it is the symbol and public market data, which is exactly what already left the machine |
...
**What never reaches the file.** Not an amount, not a description, not an institution, account, security or contract name, not an ISIN, not a payslip figure, not a rule's pattern. **The price pass is the one exception and it is exactly what already left the machine** — a `ticker` and an `exchange` (D11), with the quote that came back, which is public market data and says nothing about how much is held. `prices.http` widens that to the whole exchange and not past it: the URL carries the symbol, the body carries the same market data, and the one thing taken out of it is anything cookie-shaped, which says nothing about a quote and does not belong on disk. **Paths are not redacted**: the ledger's is already in the recent-file list in plain text, and no storage failure can be diagnosed without knowing which file it was.
