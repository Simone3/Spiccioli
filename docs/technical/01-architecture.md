...
**Before any of that, the process decides whether it is the one that runs.** `app.requestSingleInstanceLock()` is asked for at module scope, and a launch that does not get it quits without opening anything — but not before Electron has handed its command line to the process that does hold it, where it arrives as `second-instance`. **One Spiccioli, one window, one open file**: a second launch is never a second window, and what it was asked to open reaches the window that already exists instead ([§12.2](../functional/specs/12-storage.md#122-the-menu-bar-and-which-file-is-open)).
...
**Three ways a ledger arrives from outside, and one place all three end up.** macOS sends an `open-file` event, which can fire long before the application is ready and is therefore listened for from the first line of `startApplication`; Windows and Linux put the path on the command line, read by `src/main/startup/LedgerFileArguments.ts` for this launch and for the refused one alike. All three reach `src/main/startup/FileOpenRequests.ts`, which **holds the path rather than acting on it** — opening a ledger ends the session that is open, and the renderer is the only side that can finish writing that one first. **Asking for the file that is already open is answered by bringing the window forward and nothing else.** The renderer takes what is waiting, on mount and again whenever it is told there is something, and **the take clears what it returns**, so the two of them cannot open one file twice between them.

**The lock is not the coordination [§1](../functional/specs/01-premise-and-constraints.md) refuses to attempt.** It covers one machine and one login session; a folder synced between two machines still has two writers, and that case is settled where it always was — the external modification is detected, the version found on disk is copied into the backup folder, and the session in front of the user wins ([§12](../functional/specs/12-storage.md)). **Which is why the atomic write's temporary file still carries the writing process's id** (`src/main/storage/LedgerBackupNaming.ts`): the queue in `LedgerSession` keeps one run's writes apart and can keep nothing from another machine's, so the two are kept out of one temporary file by name. Two writers filling one of those interleave their bytes, and the rename puts the splice on the ledger.
...
The main process then asks the renderer to finish and waits, with a bound — but **the bound is on being idle, not on the whole close**. A renderer that cannot answer must not be able to stop the application from exiting; a renderer that is *writing* is answering, and a write has five attempts with a timeout each, which is far longer than the seconds an unresponsive one is given. Cutting that off at the idle bound would throw away the very changes the close exists to finish writing. So the clock runs only while nothing is moving — `session.isWriting()` is what says so — and a second, much longer bound covers the whole thing so that no amount of work can hold the application open forever. **That second bound is asked before anything can report progress**, or a renderer writing without end would report some at every poll and never reach it. Both are in `SHUTDOWN_CONFIG`, and the longer one is derived from the retry budget rather than guessed at.

**A close that is waiting on a person is under neither bound**, and that is the one exemption. A pause over `pauseClose` is the renderer saying it has put a question in front of the user — what to do about changes the file never took, or about a draft nothing has been written from — and a person is not a state to be timed out: giving up on one would destroy the very thing it had just asked about. The pause counts only while the window that raised it still exists, so it cannot outlive the renderer that asked for it and leave the application open with nobody left to answer.

**The close is finished when the session names no open file *and* has nothing in flight.** Both, because `closeSession` re-reads the ledger to copy it: the session stops naming its file only once that copy is on disk, and a wait that acted on the file path alone would let the process go in the middle of taking the closing backup.
...
- **The operating system is not told what a `.spiccioli` file is.** Everything that decides what happens to a ledger handed over is built ([§1.4](#14-what-the-main-process-does-at-startup)), but nothing registers the extension with macOS, Windows or Linux — so a double-click reaches none of it, and *Open with…* or a path on the command line is what does. It is out of scope for v1 ([§15](../functional/specs/15-out-of-scope.md)), being three unrelated pieces of packaging work — a document type declared in the macOS bundle, per-user registry entries on Windows, which the Squirrel maker has no way to write, and a MIME type with a `.desktop` entry on Linux — and, like the release itself, none of it can be built or exercised on one machine.
