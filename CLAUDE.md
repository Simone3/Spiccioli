...
Phases 1, 2 and 3 of [§8](docs/technical/08-implementation-plan.md) have landed: the domain model and the file, then the shell around it, then Accounts. A file can be created, opened, saved, backed up, rotated, displaced by an external write and refused when unreadable; an open file lands on Portfolio, the sidebar reaches all eight screens, Settings and Accounts are finished, and every other screen is a shell showing the empty state of [§14](docs/functional/specs/14-empty-and-error-states.md). The kit those screens are built from is `src/components/common/`; phase 4 is Transactions.
