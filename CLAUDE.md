...
- Do NOT introduce extra libraries unless you justify them briefly and they clearly reduce work or risk. The runtime set v1 adds is settled and is four: `react-router`, `react-datepicker` behind our own date control, `@dnd-kit/react` and `recharts`, each installed in the phase that needs it ([§8.2](docs/technical/08-implementation-plan.md#the-dependencies-d7--d10-add)). Each one stays wrapped in a component of ours, so nothing outside that component imports it.
...
- The ledger is **one JSON document** with the extension `.spiccioli`, read whole and written whole ([§8.2](docs/technical/08-implementation-plan.md#82-decisions)). It needs no persistence dependency and must not acquire one: the model lives in memory in the renderer because every figure in [§11](docs/functional/specs/11-calculations.md) is derived on read, so the file is only ever parsed and serialized. Do not reach for a database, an ORM or a serialization library.
- Money, quantities and rates are **integers in minor units** ([§8.2](docs/technical/08-implementation-plan.md#what-d3-fixes)): amounts in cents, quantities, unit prices and rates in ten-thousandths. A stored figure is never a fractional `number`; a product widens to the working scale of eight decimal places and stays exact there, and **a division is the only inexact step** — it rounds half away from zero, as do the three mid-calculation roundings [§11](docs/functional/specs/11-calculations.md) names and the display. Conversion happens once at each boundary — the amount field, the import parser, the reader and the writer — and the reader refuses a figure that is not an integer at its field's scale. The bisection of [§11.8](docs/functional/specs/11-calculations.md#118-annualised-return) is the one place binary64 is used, because the specification requires it. No `bigint` and no decimal library.
- Every save is **the whole file, atomically**: written to a temporary file in the same directory and renamed over the old one. Before each one, the file's SHA-256 is re-computed and compared against what was recorded at the last read or write; a mismatch is an external modification and is handled as [§12](docs/functional/specs/12-storage.md) says, never ignored and never resolved by asking the user to choose.
...
- Nothing leaves the machine except a security's `ticker` and `exchange`, sent to the price provider when the user presses *Update prices* ([§7.6](docs/functional/specs/07-investments.md)). Not the ISIN, not an amount, not a quantity, not an account. No telemetry, no crash reporting service, no update check.
- The price provider is **Yahoo Finance's `v8/finance/chart`**, keyless, one request per listing, and it lives behind one adapter in the main process that nothing else imports ([§8.2](docs/technical/08-implementation-plan.md#82-decisions) D11). The `exchange` enum is ours and the Yahoo suffix table belongs to that adapter alone — **a suffix never reaches the file**. A quote is written only after the four refusals of [§7.6](docs/functional/specs/07-investments.md#76-prices) have been applied: future-dated, zero or less, not EUR, or no currency stated.
