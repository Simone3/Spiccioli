...
Phases 1 to 9 of [§8](docs/technical/08-implementation-plan.md) have landed: the domain model and the file, then the shell around it, then Accounts, then Transactions, then Bulk import, then Categories, then Investments, then Update prices, then Salaries. A file can be created, opened, saved, backed up, rotated, displaced by an external write and refused when unreadable; an open file lands on Portfolio, the sidebar reaches all eight screens, Settings, Accounts, Transactions, the import reached from it, Categories, Investments and Salaries are finished, and Portfolio and Checks are shells showing the empty state of [§14](docs/functional/specs/14-empty-and-error-states.md). The kit those screens are built from is `src/components/common/`, and the one file that imports the charting library is `LineChart.tsx` inside it; the one thing in the application that touches the network lives behind one interface in `src/main/prices/`. Phase 10 is *Matching and checks*, which needs no new dependency.
...
- One dark theme, no light theme and no theme switch. Colors come from the variables in `src/index.css` and are never written inline. A chart names a series by its number and never by a colour, the five series variables being fixed there too ([§6.2](docs/technical/06-styling.md#62-one-theme-and-it-is-dark)).
