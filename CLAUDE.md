...
- Do NOT introduce extra libraries unless you justify them briefly and they clearly reduce work or risk. The runtime set is settled and is five, and **all five are installed**: `react-router`, `react-datepicker` behind our own date control, dnd kit behind the rule list — taken as `@dnd-kit/core` with `@dnd-kit/sortable` — `recharts` behind `src/components/common/LineChart.tsx` and `src/components/common/PieChart.tsx`, and `pdfjs-dist` behind `src/main/import/PdfText.ts` ([§8.3](docs/technical/08-decisions.md#83-the-dependencies-the-application-adds)). Each one stays wrapped in a module of ours, so nothing outside it imports the library.
- `package.json` dependencies must use exact versions. No `^` or `~`.
- **Reading a bank export is ours and takes no dependency.** The `.xlsx` and delimited readers behind the import's *Upload* live in `src/main/import/`, and a spreadsheet or CSV parsing library must not be installed to replace them: every one of them hands back a JavaScript `number` for an amount cell, which puts a rounding between the file and the import parser that [§8.2](docs/technical/08-decisions.md#82-what-d3-fixes) fixes as the one conversion boundary. A cell crosses to the renderer as **the literal text the file spells it with** ([§8.3](docs/technical/08-decisions.md#83-the-dependencies-the-application-adds)). The import templates themselves are code in `src/logic/import/`, never configuration, a template file or an editor ([§15](docs/functional/specs/15-out-of-scope.md)) — and that holds for the payslip templates beside them.
- **A payslip PDF is the one import shape that is not ours to read**, and `pdfjs-dist` behind `src/main/import/PdfText.ts` is what reads it ([§8.3](docs/technical/08-decisions.md#83-the-dependencies-the-application-adds)). That module is the only thing in the application that may import the library, and what it hands back is the lines of text the document prints **and the point each piece of them starts at across the page** — a payroll form is a grid of boxes, and which heading a figure sits under is said by position and by nothing else. The boundary argument above is untouched either way: a figure still crosses as the characters the page spells it with, a position is no more read on the way across than a cell's text is, and the conversion to cents still happens in the one parser. **Nothing is rendered and nothing leaves the machine**: only the text layer is asked for, and the library is told it may not fetch character maps or font data. `scripts/electron-bundle.js` copies `pdf.worker.mjs` beside the main bundle, because esbuild cannot see the import that loads it.
